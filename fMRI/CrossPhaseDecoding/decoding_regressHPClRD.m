% Control analysis: vlPFC decoding depends on compressed hippocampal representations.
%- Regress out low-dimensional hippocampal PCA components from vlPFC activity
%- Test whether cross-phase decoding accuracy in vlPFC remains above chance
...
for i_subj=1:39
    ...
    ds_HSC_hip = cosmo_fmri_dataset(data_HSC,'mask',mask1_fn);
    d_HSC_hip = ds_HSC_hip.samples;
  
    [coeff,score,latent,tsquared,explained,mu]=pca(d_HSC_hip);
    ...
    compressed_hsc = score(:,1:n);
    
    
    data_LSC=fullfile(sub_s1_path,'glm_LSC.nii');
    ds_LSC_hip = cosmo_fmri_dataset(data_LSC,'mask',mask1_fn);
    d_LSC_hip = ds_LSC_hip.samples;
  
    [coeff,score,latent,tsquared,explained,mu]=pca(d_LSC_hip);
    ...
    [m,u] = size(ds_hsc_s1.samples); % m = trials, u=voxles
    x_aug = [ones(m,1),compressed_hsc];
    B = x_aug \ ds_hsc_s1.samples;
    Y_fit = x_aug * B;
    ds_hsc_s1.samples = ds_hsc_s1.samples -Y_fit;

    [m,u] = size(ds_lsc_s1.samples); % m = trials, u=voxles
    x_aug = [ones(m,1),compressed_lsc];
    B = x_aug \ ds_lsc_s1.samples;
    Y_fit = x_aug * B;
    ds_lsc_s1.samples = ds_lsc_s1.samples -Y_fit;
