...
spm_jobman('initcfg');
spm('defaults', 'fMRI');
data_dir = 'L:\FLXX_1\First_level\Pro_proc_data';
sublist = dir([data_dir,'\sub*']);
%%
mydir = 'H:\GJXX_1_reanalysis\no_too_easyorhard_item\events';

for nsub=1:24 
     sub_dir = [data_dir,filesep,sublist(nsub).name];
     onset_dir = [mydir,filesep,sublist(nsub).name];
    %-----------------------------------------------------------------------
    out_dir = ['H:\GJXX_1_reanalysis\no_too_easyorhard_item\glm_item_hl\',sublist(nsub).name]; %
    ...
    HSC=all_con.onset(string(all_con.trial_type) == 'e' & all_con.condition_median_jiaocha_1== 1);
    LSC=all_con.onset(string(all_con.trial_type) == 'e' & all_con.condition_median_jiaocha_1== -1);
    tianchong=all_con.onset(string(all_con.trial_type) == 't'| all_con.condition_median_jiaocha_1== 0);
